...
def checkSysArgv(args):
	if len(args) != 3:
		sys.exit('Usage: thumbgen_py [path to source folder] [thumb size]')
...
def processFiles(files, width, height, dst):
        """
	...
	for file in files:
		img = Image.open(file)
                ...
                thumb = makeThumb(img, size=size)
                ofname = os.path.basename(file)
                ofname = os.path.splitext(ofname)[0]
                thumb.save(dst + '/' + ofname + '.png', "PNG")
	return thumbs
...
def main(args):
	...
	srcURL = args[1]
	dstURL = srcURL + '/thumbs'
        thumb_target_size = args[2]
        checkTargetSize(thumb_target_size)
	folderExists(srcURL)
	folderExists(dstURL)
	files = getFileList(srcURL)
	thumbs = processFiles(files, int(thumb_target_size), int(thumb_target_size), dstURL)
